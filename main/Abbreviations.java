...
public class Abbreviations {
	...
	public static Proposition pow(Term base, Term pow, Term res) {
		...
		Term.Var prev = new Term.Var(free++);
		Term.Var previndex = new Term.Var(free++);
		...
		Proposition induct = new Quantifier.Universal(new Quantifier.Universal(new AtomicProposition.Implies(new AtomicProposition.And(ante, valid), next), prev.id), previndex.id);
		...
		return new Quantifier.Existential(new Quantifier.Existential(all, bound.id), sequence.id);
	}
